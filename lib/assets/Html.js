...
extendWithGettersAndSetters(Html.prototype, {
    ...
    _reformatParseTree: function (removeWhiteSpaceNodes, removeComments) {
        ...
        while (q.length) {
            var node = q.shift();
            // Leave conditional comments, Knockout.js containerless bindings, and SSIs alone even if removeComments is true:
            if ((removeComments && node.nodeType === node.COMMENT_NODE && !/^#|^\[if|^<!\[endif\]|^\s*\/?ko\s/.test(node.nodeValue)) ||
                ...
                if (node.nodeType === node.ELEMENT_NODE) {
                    ...
                    if (hasTextChild) {
                        ...
                        for (var i = 0 ; i < node.childNodes.length - 1 ; i += 1) {
                            // Collapse neighbouring text nodes:
                            if (node.childNodes[i].nodeType === node.TEXT_NODE && node.childNodes[i + 1].nodeType === node.TEXT_NODE) {
console.warn("collapsing! >>" + node.childNodes[i].nodeValue + "<< >>" + node.childNodes[i + 1].nodeValue + "<<");
                                node.childNodes[i].nodeValue += node.childNodes[i + 1].nodeValue;
                        ...
                        for (var i = 0 ; i < node.childNodes.length ; i += 1) {
                            if (node.childNodes[i].nodeType === node.TEXT_NODE) {
var before =                                 node.childNodes[i].nodeValue;
var after = node.childNodes[i].nodeValue.replace(/^\s*|\s*$/g, '').replace(/\s+/g, ' ');
if (before !== after) {
console.warn("trimming >>" + before + "<< >>" + after + "<<");
}


                                node.childNodes[i].nodeValue = node.childNodes[i].nodeValue.replace(/^\s*|\s*$/g, '').replace(/\s+/g, ' ');
                            }
                        }
                    }
                }
